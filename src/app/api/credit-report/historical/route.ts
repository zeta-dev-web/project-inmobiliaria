...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `https://api.bcra.gob.ar/centraldedeudores/v1.0/Deudas/Historicas/${cuilNumeros}`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        errorMessage = 'No se encontraron datos históricos para el CUIL ingresado';
    ...
    console.error('Error consulting BCRA historical:', error);
    return NextResponse.json(
