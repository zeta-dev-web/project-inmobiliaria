...
import { Plus, X, Home, User, Users, DollarSign, Calendar, Clock } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
export function RentalModal({ open, onOpenChange, rental }: RentalModalProps) {
  ...
  const { data: properties = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.data.filter((p: Property) => 
        p.type === "RENT" && (p.status === "AVAILABLE" || (rental && p.id === rental.propertyId))
      );
    },
  });
  ...
  useEffect(() => {
    if (rental) {
      ...
      setValue("tenantId", rental.tenantId);
      setValue("landlordId", rental.landlordId);
  ...
  const createMutation = useMutation({
    mutationFn: (data: FormData & { guarantorIds: string[] }) => clientAxios.post("/rentals", data),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      if (error.response?.data?.errors) {
        ...
        toast.error(error.response?.data?.message || "Error al crear el alquiler");
      }
    },
  });

  const updateMutation = useMutation({
    mutationFn: (data: FormData & { guarantorIds: string[] }) =>
    ...
    onError: (error: any) => {
      if (error.response?.data?.errors) {
        ...
        toast.error(error.response?.data?.message || "Error al actualizar el alquiler");
      }
    },
  });
  ...
  const onSubmit = (data: FormData) => {
    setBackendErrors({});
    const formData = { ...data, guarantorIds };
    if (rental) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-gray-900">Información de la Propiedad</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                {backendErrors.propertyId && (
                  <p className="text-sm text-red-500">{backendErrors.propertyId}</p>
                )}
              </div>

              <div className="space-y-2">
                <Label className="text-sm font-medium">Precio de Alquiler *</Label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder={selectedProperty ? selectedProperty.price.toString() : "0.00"}
                    className="pl-10"
                  />
                </div>
                {selectedProperty && (
                  <p className="text-xs text-gray-500">Precio publicado: ${selectedProperty.price.toLocaleString()}</p>
                )}
                {backendErrors.rentalPrice && (
                  <p className="text-sm text-red-500">{backendErrors.rentalPrice}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-gray-900">Partes del Contrato</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="text-sm font-medium">Inquilino (Locatario) *</Label>
                <Controller
                  name="tenantId"
                  control={control}
                  render={({ field }) => (
                ...
                />
                {backendErrors.tenantId && (
                  <p className="text-sm text-red-500">{backendErrors.tenantId}</p>
                )}
              </div>

              <div className="space-y-2">
                <Label className="text-sm font-medium">Propietario (Locador) *</Label>
                <Controller
                  name="landlordId"
                  control={control}
                  render={({ field }) => (
                    <SearchableSelect
                      options={availableLandlordOptions}
                      value={field.value}
                      onChange={field.onChange}
                ...
                />
                {backendErrors.landlordId && (
                  <p className="text-sm text-red-500">{backendErrors.landlordId}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2 mt-4">
              ...
              {backendErrors.guarantorIds && (
                <p className="text-sm text-red-500">{backendErrors.guarantorIds}</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  type="date"
                  value={watch("startDate") ? new Date(new Date(watch("startDate")).getTime() - new Date(watch("startDate")).getTimezoneOffset() * 60000).toISOString().split('T')[0] : ''}
                  onChange={(e) => {
                ...
                {backendErrors.startDate && (
                  <p className="text-sm text-red-500">{backendErrors.startDate}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  type="date"
                  value={watch("endDate") ? new Date(new Date(watch("endDate")).getTime() - new Date(watch("endDate")).getTimezoneOffset() * 60000).toISOString().split('T')[0] : ''}
                  onChange={(e) => {
                    const localDate = new Date(e.target.value + 'T12:00:00');
                ...
                {backendErrors.endDate && (
                  <p className="text-sm text-red-500">{backendErrors.endDate}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                {backendErrors.paymentDueDay && (
                  <p className="text-sm text-red-500">{backendErrors.paymentDueDay}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="text-sm font-medium">Actualización (meses) *</Label>
                <div className="relative">
                ...
                {backendErrors.updateFrequency && (
                  <p className="text-sm text-red-500">{backendErrors.updateFrequency}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                {backendErrors.lateFee && (
                  <p className="text-sm text-red-500">{backendErrors.lateFee}</p>
                )}
              </div>
            </div>
            ...
            <div className="mt-4">
              ...
              <div className="flex gap-2 mt-2">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    placeholder={watch("administrationType") === "PERCENTAGE" ? "10" : "0.00"}
                    className="pl-10"
                  />
                </div>
                <Controller
              ...
              {backendErrors.administrationAmount && (
                <p className="text-sm text-red-500 mt-1">{backendErrors.administrationAmount}</p>
              )}
            </div>
          </div>
