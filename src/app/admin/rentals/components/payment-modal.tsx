...
export function PaymentModal({ open, onOpenChange, rental }: PaymentModalProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      clientAxios.get("/users?role=ADMIN")
        .then(({ data }) => {
  ...
  useEffect(() => {
    if (rental && open) {
      ...
      const currentPeriod = rental.pricePeriods?.find((p: any) => 
        p.startMonth <= currentContractMonth && p.endMonth >= currentContractMonth
      );
  ...
  const isLastMonth = () => {
    ...
    return endDate.getFullYear() === parseInt(year) && endDate.getMonth() === parseInt(month) - 1;
  };
  ...
  useEffect(() => {
    const subscription = watch((value) => {
      ...
      if (paymentDate && periodMonth && rental?.lateFee && rental?.paymentDueDay) {
        const paymentDateObj = new Date(paymentDate);
        const [year, month] = periodMonth.split("-");
        const dueDate = new Date(parseInt(year), parseInt(month) - 1, rental.paymentDueDay);
        
        if (paymentDateObj > dueDate) {
          const firstDayOfMonth = new Date(parseInt(year), parseInt(month) - 1, 1);
          const daysDiff = Math.floor((paymentDateObj.getTime() - firstDayOfMonth.getTime()) / (1000 * 60 * 60 * 24)) + 1;
  ...
  }, [applyLateFee, calculatedLateFee, items, rental, open, setValue, currentPeriodPrice, watch]);
        ...
        {showConfirmation ? (
          <div className="space-y-6">
            {isLastMonth() && (
              <div className="bg-red-50 border border-red-300 rounded-lg p-4 text-red-800">
                ...
                <p className="text-sm">Recordar notificar al inquilino que debe entregar la llave el día 01 del mes siguiente al vencimiento.</p>
              </div>
            )}
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <h3 className="font-semibold text-gray-900 mb-3">Confirmar Asiento de Pago</h3>
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{new Date(watch("periodMonth") + "-01").toLocaleDateString('es-ES', { year: 'numeric', month: 'long' })}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">${(() => {
                    ...
                    return (currentPeriod?.price || rental.rentalPrice).toLocaleString();
                  })()}</span>
                </div>
                {items.length > 0 && (
                  <>
                    <div className="border-t pt-2 mt-2">
                      <span className="text-gray-600 font-medium">Items Adicionales:</span>
                    </div>
                ...
                {applyLateFee && calculatedLateFee > 0 && (
                  <>
                    ...
                    <div className="flex justify-between pl-4">
                      ...
                      <span className="font-medium">${calculatedLateFee.toLocaleString()}</span>
                    </div>
                  </>
                )}
                <div className="flex justify-between border-t pt-2 mt-2">
                  <span className="text-gray-900 font-semibold">Total a Pagar:</span>
                  <span className="text-[#600096] font-bold text-lg">${watch("amount")?.toLocaleString()}</span>
                </div>
                {watch("notes") && (
                  <div className="border-t pt-2 mt-2">
                ...
                <div className="flex justify-between border-t pt-2 mt-2">
                  ...
                  <span className="font-medium">{admins.find((a: any) => a.id === watch("signedById"))?.name}</span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Pago Asentado Exitosamente</h3>
              <p className="text-gray-600">El pago ha sido registrado correctamente en el sistema.</p>
            </div>
          ...
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
            ...
            <div className="space-y-1 text-sm">
              ...
              <p><span className="font-medium">Precio Base:</span> ${rental.rentalPrice.toLocaleString()}</p>
              <p><span className="font-medium">Precio Actual:</span> ${currentPeriodPrice.toLocaleString()}</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-4 h-4"
                  />
                  <span className="text-sm font-medium text-gray-900">Aplicar Mora: ${calculatedLateFee.toLocaleString()}</span>
                </div>
                <span className="text-xs text-gray-600">({Math.floor((new Date(watch("paymentDate")).getTime() - new Date(watch("periodMonth") + "-01").getTime()) / (1000 * 60 * 60 * 24)) + 1} días x ${rental.lateFee})</span>
              </div>
            )}
            ...
            {(items.length > 0 || (applyLateFee && calculatedLateFee > 0)) && (
              <div className="space-y-2 mt-2">
                ...
                {applyLateFee && calculatedLateFee > 0 && (
                  <div className="flex items-center justify-between bg-yellow-50 p-2 rounded border border-yellow-200">
                    ...
                    <span className="text-sm font-medium">${calculatedLateFee.toLocaleString()}</span>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="space-y-2">
            ...
            <Select onValueChange={(value) => setValue("signedById", parseInt(value))}>
              <SelectTrigger>
                <SelectValue placeholder={loadingAdmins ? "Cargando..." : "Seleccionar usuario admin..."} />
              </SelectTrigger>
              <SelectContent>
                {loadingAdmins ? (
                  <SelectItem value="loading" disabled>Cargando...</SelectItem>
                ) : admins.length === 0 ? (
                  <SelectItem value="empty" disabled>No hay usuarios admin</SelectItem>
                ) : (
