...
import { MoreVertical, Edit, Trash2, Eye, DollarSign, Receipt } from "lucide-react";
...
export default function RentalsPage() {
  ...
  const filteredData = search && data.length > 0 ? data.filter((rental: RentalWithRelations) => {
    const matchesSearch = 
      rental.property.name.toLowerCase().includes(search.toLowerCase()) ||
      rental.property.address.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const handlePayment = async (rental: RentalWithRelations) => {
    const { data: rentalsData } = await clientAxios.get(`/rentals?page=${page}&limit=10`);
  ...
  const getCurrentPrice = (rental: RentalWithRelations) => {
    ...
    const currentPeriod = rental.pricePeriods.find(p => 
  ...
  const columns = [
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="font-semibold text-gray-900 truncate">{rental.property.name}</p>
              <p className="text-sm text-gray-500 truncate">{rental.property.address}</p>
            </div>
          </div>
          {rental.warnings && (
    ...
    },
    {
      ...
      render: (rental: RentalWithRelations) => {
        ...
        const isPaid = (rental as any).payments?.some((p: any) => 
          p.periodMonth === currentPeriodMonth
        );
    ...
    return (
      <div className="p-4 md:p-6 space-y-6">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">🏠 Alquileres</h1>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <p className="text-yellow-800">No hay alquileres registrados aún. Crea el primero usando el botón "Nuevo Alquiler".</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        ...
        <div className="flex gap-2">
          ...
          >
            Notificaciones ({data.filter((r: any) => {
              if (!r.warnings) return false;
              const hasUnnotifiedContractExpiring = r.warnings.contractExpiring && 
                !r.notifications?.some((n: any) => n.type === 'CONTRACT_EXPIRING' && n.notified);
              const hasUnnotifiedPriceUpdate = r.warnings.priceUpdate && 
                !r.notifications?.some((n: any) => n.type === 'PRICE_UPDATE' && n.notified);
      ...
      <ModernTable
        ...
        columns={columns}
        isLoading={isLoading}
        emptyMessage={search ? "No se encontraron alquileres con los filtros aplicados" : "No hay alquileres registrados"}
        getRowClassName={(rental) => {
          const hasExpiring = rental.warnings?.contractExpiring;
          const hasPriceUpdate = rental.warnings?.priceUpdate;
          return hasExpiring ? 'border-2 border-red-500' : hasPriceUpdate ? 'border-2 border-blue-500' : 'border border-gray-200';
        }}
        actions={(rental) => (
        ...
        extras={(rental) => (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48 bg-white">
              <DropdownMenuItem onClick={() => handleView(rental)} className="cursor-pointer">
              ...
              <DropdownMenuItem onClick={() => handleEdit(rental)} className="cursor-pointer">
      ...
      {selectedRental && (
        <PaymentHistoryModal
          ...
          tenantPhone={(selectedRental as any).tenant?.phone}
        />
      )}
